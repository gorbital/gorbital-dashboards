...
|---|---|---|---|---|
| `apps/devtools` | **Dev Portal** | inspect the app on your bench while `aps dev` runs it | devtools.apistock.dev | 3100 |
| `apps/observe` | **Observability Portal** | requests, traces, errors, jobs, mail and logs from the data the app already sends | observe.apistock.dev | 3200 |
| `apps/deploy` | **Deployment Portal** | releases, rollouts, migrations, the fleet and its health | deploy.apistock.dev | 3300 |
...
`packages/ui` (`@apistock/dash`) holds the dashboard shell in three variants
