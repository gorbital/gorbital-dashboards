...
export function Settings() {
  ...
  const onTab = useCallback((v: string | null) => setTab(v === "flags" ? "flags" : "settings"), []);
  const onFlag = useCallback((v: string | null) => setSelectedFlag(v), []);
  ...
  const pickTab = (t: Tab) => {
    setTab(t);
    setQueryParam("tab", t === "settings" ? null : t);
  };
  const selectFlag = useCallback((key: string | null) => {
    setSelectedFlag(key);
    setQueryParam("flag", key);
  }, []);
  ...
  return (
    <>
      <Suspense fallback={null}>
        ...
        <QueryParam name="flag" onValue={onFlag} />
      </Suspense>
      <PageHeader product="devtools" title="Settings" description={tab === "flags" ? "feature flags declared in code, stored when changed, applied live · /ops/flags" : settings.data ? `${all.length} runtime settings · ${modified} modified · /ops/settings` : "declared in code, stored when changed, applied live"}>
        <Segmented<Show>
          options={[
            { value: "all", label: "All" },
      ...
      <Page>
        <Tabs<Tab>
          ...
          onChange={pickTab}
          tabs={[
            { value: "settings", label: "Settings", badge: settings.data ? all.length : undefined },
        ...
        >
          <TabPanel value="flags">
            <Gate need="ops" loading={<Table<OpsSetting> columns={[]} rows={[]} rowKey={(s) => s.key} loading />}>
              <Flags selectedKey={selectedFlag} onSelect={selectFlag} show={show} />
            </Gate>
          </TabPanel>
